...
class MainForm(npyscreen.FormBaseNew):
  ...
  def create(self):
    ...
    self.healthBarObj = self.add(statBox.StatSlider, name='HP', color='DANGER', max_width = bar_width, max_height=bar_height,
                                 relx=bar_xs[0], rely=2, editable=False,
                                 contained_widget_arguments=bar_args)

    bar_args={'out_of': 100, 'value':40, 'color':'GOOD'}
    self.xpbarObj = self.add(statBox.StatSlider, name='XP', color='GOOD', max_width = bar_width, max_height=bar_height,
    ...
    for idx, (stat, val) in enumerate(character.DEFAULT_BATTLE_STATS.items()):
      self.statObjs[stat] = self.add(statBox.StatBox, name=stat.upper(), values=[str(val)], color='NO_EDIT', editable=False,
    ...
    for idx, (person, val) in enumerate(character.DEFAULT_PERSON_STATS.items()):
      self.personObjs[person] = self.add(statBox.StatBox, name=person.upper(), values=[str(val)], color='NO_EDIT', editable=False,
                                         max_width=person_width, max_height=person_height, relx=person_xs[idx], rely=person_y)
  ...
  def event_input_send(self):
    ...
    commandHandler.CommandHandler.parse_command(self, text)
  ...
  def update_character(self):
    player = self.find_parent_app().resources.get_player(resourceManager.ME)

    self.xpbarObj.value = player.xp
    self.xpbarObj.out_of = player.xp
    self.healthBarObj.value = player.health
    self.healthBarObj.out_of = player.max_health
